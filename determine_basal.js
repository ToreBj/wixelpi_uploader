...
function init() {
    ...
    determinebasal.determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, offline) {
        ...
        var bgifactor = Math.round( (((bg - 180 )/(400-180))+0.5 )*100)/100;
        console.error("bgifactor: " + bgifactor);
        ...
        var threshold = profile.min_bg - 10;
        ...
        if (eventualBG < profile.min_bg) { // if eventual BG is below target:
            ...
            if (glucose_status.delta > bgi*bgifactor && glucose_status.avgdelta > bgi*bgifactor) {
                rT.reason += ", but Delta " + tick + " > BGI " + bgi + " * " + bgifactor;
        ...
        // if eventual BG is above min but BG is falling faster than BGI * 0.65
        if (glucose_status.delta < bgi*bgifactor || glucose_status.avgdelta < bgi*bgifactor) {
            rT.reason = "Eventual BG " + eventualBG + ">" + profile.min_bg + " but Delta " + tick + " < BGI " + bgi + " * " + bgifactor;
            if (currenttemp.duration > 0) { // if there is currently any temp basal running
                rT.reason = rT.reason += "; cancel";
                return determinebasal.setTempBasal(0, 0, profile, rT, offline); // cancel temp
            }
